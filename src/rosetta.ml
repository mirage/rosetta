...
type 'kind pack = V : ('kind, 'decoder) tag * 'decoder -> 'kind pack
...
let src { pack = V (kind, decoder); _ } source off len = match kind with
  | UTF_7 -> Yuscii.src decoder source off len
  | ISO8859 -> Uuuu.src decoder source off len
  | KOI8 -> Coin.src decoder source off len

let decode { pack = V (kind, decoder); _ } = match kind with
  | UTF_7 -> Yuscii.decode decoder
  | ISO8859 -> Uuuu.decode decoder
  | KOI8 -> Coin.decode decoder

let decoder
    : ([< encoding ] as 'kind) -> src -> 'kind decoder
  = fun kind src ->
  match kind with
  | #Uuuu.encoding as k ->
     { src; pack = V (ISO8859, Uuuu.decoder k src); }
  | #Coin.encoding as k ->
     { src; pack = V (KOI8, Coin.decoder k src); }
  | #yuscii_encoding ->
...
let decoder_kind { pack = V (kind, decoder); _ } = match kind with
  | UTF_7 -> `UTF_7
